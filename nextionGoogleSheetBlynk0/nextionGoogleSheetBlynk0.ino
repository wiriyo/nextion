...
char ssid[] = "Hi-Tech FL2@2.4G";
char pass[] = "3014030140";
int wifiFlag = 0;
#define wifiLed 2  //D2
...
//page id 2 welcome
NexVariable tempval = NexVariable(2, 4, "tempval");
NexVariable humval = NexVariable(2, 5, "humval");
NexVariable gtemp = NexVariable(2, 6, "gtemp");
NexVariable ghum = NexVariable(2, 7, "ghumid");
...
void setup() {
  ...
  timer.setInterval(60000L, sendData2GGSheet);  //ส่งค่าเซ็นเซอร์ขึ้น google sheet ทุกๆ 10 วินาที
  timer.setInterval(60000L, xymdtoBlynk);
}
//=================Setup Function===============//
// float temp1;
// float humi1;
...
void xymdtoNex() {
  uint8_t result1;
  temp1 = (node1.getResponseBuffer(0) / 10.0f);
  humi1 = (node1.getResponseBuffer(1) / 10.0f);
  ...
  //Page 3 Text XY-MD02
  String command = "tempval.txt=\"" + String(temp1) + "\"";
  Serial.print(command);
  endNextionCommand();

  String command1 = "tempval.txt=\"" + String(temp1) + "\"";
  Serial.print(command1);
  endNextionCommand();

  String command2 = "humval.txt=\"" + String(humi1) + "\"";
  ...
  Serial.print("gtemp.val=");  //Send the object tag
  Serial.print(val1);          //Send the value
  endNextionCommand();

  int val2 = map(humi1, 0, 100, 0, 180);
  Serial.print("ghumid.val=");  //Send the object tag
